...
.pull-right[
**ROC-curve:** Step function that is created by gradually lowering (from `+inf` to `-inf`) the cutoff (*c*) for classifying a sample as positive. Hence, it aggregates all conceivable confusion matrices and creates a performance profile of the classifier. [[5](#references)]
...
**Lower right triangle:** A classifier that performs worse than random. Technically, such a classifier does not exist as you could simply invert the predictions (e.g., FP become TN).
]
