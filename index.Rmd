...
.pull-right[
**ROC-curve:** Step function that is created by gradually lowering the cutoff (*c*) for classifying a sample into the positive class (from `+inf` to `-inf`). Hence, it is an aggregation of all confusion matrices that result from different values of *c*. [[5](#references)]
